...
class AI_Agent:
    ...
    def get_action(self, game_obj) -> int:
        ...
        with torch.no_grad():
            ...
            # 在 logits 层进行 masked-argmax，而不是依赖 probs
...
class Game:
    ...
    def __init__(self, model_path: str = None, first_player: str = None, match_mode: str = 'single'):
    ...
    def _play_win_sound_once(self):
        ...
        try:
            ...
            try:
                ...
                print(f"[Audio] Channel reservation/setup failed (非致命): {e}")
    ...
    def _reset_for_next_round(self):
        ...
        # 不自动发牌：展示 scoreboard 覆盖层，等待玩家点击 Start Match
        self.show_scoreboard_overlay = True
    ...
    def _on_round_victory(self, winner_player):
        ...
        # 非终局或普通单局：先立即展示本局胜者的大字并播放一次胜利音效
        ...
        # 否则交替先手并准备下一局（显示 Start Match 覆盖层，等待玩家点击）
    ...
    def run(self):
        ...
        while running:
            ...
            # 在 BO5 的 Start Match 覆盖层显示期间，禁止 AI 自动行动，避免修改手牌或状态
            if not self.show_scoreboard_overlay and self.current_player == self.player2:
                # 只有当不在显示 scoreboard overlay 时才让 AI 行动，避免视觉和状态冲突
