...
class DemonAgent_V5:
    ...
    def _evaluate_decision_tree(self, state):
        ...
        # 四代恶魔的核心策略极其简单：防守是不需要的
        # 因为如果能够直接一波杀死对手那就无需考虑下一波的防守
        # 而一波杀死的关键就是蓝区平方数
        # 这和二代恶魔不同。二代恶魔是个莽夫，只考虑到用攻击威慑对手，然而‘威慑’本身是建立在害怕的前提上
        return self.normal_dealing(state, r3, r5_real, r5_empty)
    # ----------------------------------------------------
    # 分支逻辑
    # ----------------------------------------------------
    def normal_dealing(self,s, r3, r5_real, r5_empty):
        # 蓝区无损攻击
