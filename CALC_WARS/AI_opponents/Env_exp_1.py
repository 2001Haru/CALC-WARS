...
class Game:
    ...
    def _get_state(self):
        ...
        state_vector[56] = self.round_number / 5.0
        state_vector[57] = self.round_number ** 2 / 25.0
        ...
        state_vector[72:79] = self.reachable_check(partial_result).astype(np.float32)
        ...
        state_vector[97:117] = expr_features
    ...
    def reachable_check(self,partial_result=None):
        """检查目标区域的可达性"""
        if partial_result is None:
            return np.zeros(7, dtype=np.int8)
        
        reachable = np.zeros(7, dtype=np.int8)
        for i, zone in enumerate(
            [self.target.red_zone] + self.target.yellow_zone + self.target.blue_zone
        ):
            for j in range(14):
                if self.current_player.hand[j]>0:
                    if self.current_player.hand[17]>0 and j != 0 and abs(partial_result / j - zone) < 1e-6:
                        reachable[i] == 1
                    elif self.current_player.hand[14]>0 and abs(partial_result + j - zone) < 1e-6:
                        reachable[i] == 1
                    elif self.current_player.hand[15]>0 and abs(partial_result - j - zone) < 1e-6:
                        reachable[i] == 1
                    elif self.current_player.hand[16]>0 and abs(partial_result * j - zone) < 1e-6:
                        reachable[i] == 1
    ...
    def step(self, action):
        ...
        if action < 20 :  # 选择手牌
            ...
            if self.current_player.skill_cards[skill_index] > 0:
                ...
                if skill_index == 0 and self.current_player.hp <100:
                    reward += 40.0  #HEAL
                else:
                    reward -= 20.0
                if skill_index == 1 or skill_index == 2:
                    reward += 50.0  #STEAL DRAW
                if skill_index == 3:
                    if self.current_player.shield_count <= 1:
                        reward += 40.0  #SHIELD
                    else:
                        reward -= 20.0
                if skill_index == 4:
            ...
            reward += -15.0 * self.round_number ** 0.8
            ...
            if len(self.selected_cards) >= 3:
                ...
                if result is not None and result >= 0:
                    ...
                    # 奖励基于造成的伤害
                    reward += 10.0  # 基础出牌奖励
                    reward += actual_damage * 6.0   #伤害奖励

                    cards_used = len(self.selected_cards)
                    efficiency = actual_damage / max(cards_used, 1)
                    reward += efficiency * 2.0  # 牌效奖励

                    distance = abs(self.target.red_zone - result)
                    reward += 1/(distance + 1) * 120.0  # 击中红区奖励
        ...
        if self.player1.hp <= 0 or self.player2.hp <= 0:
            ...
            if self.current_player.hp > 0:
                reward = 1800  - 1.4 ** (self.step_count / 25.0) # 获胜奖励 鼓励速胜
            else:
                reward = -800.0  # 失败惩罚
