...
from Game_env_PPO_ver7 import Game, CardType, OperatorType, Card
from typing import List, Dict, Tuple, Optional 


class PPOTransformer(nn.Module):
    """混合架构: MLP处理静态特征 + Transformer处理序列"""
    def __init__(self, obs_dim=104, action_dim=29, hidden_size=512, 
    ...
    def get_features(self, x: torch.Tensor):
        ...
        addition_features = x[:, 97:104]
        static_features = torch.cat([static_features, addition_features], dim=-1)
        
        # 拆分序列
        seq_tokens = sequence_compressed.reshape(-1, self.seq_len, 2)
        card_indices = (seq_tokens[:, :, 0] * 20).long().clamp(0, 20) 
...
class TrainingManager:
    ...
    def __init__(self, env, config: Dict[str, Any]):
        ...
        self.pretrain_steps = config.get('pretrain_steps', 30000)
        ...
        self.stage_boundaries = [4000, 8000]  # 阶段切换的episode阈值
