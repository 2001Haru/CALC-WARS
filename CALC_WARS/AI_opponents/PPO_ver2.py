    ...
    def update(self, trajectories_batch: List[List[Dict]], entropy_coef: float = None) -> float:
        ...
        for _ in range(self.ppo_epochs):
            ...
            for trajectory in trajectories_batch:
                ...
                logits_last, values_seq, _ = self.policy_net(states_seq, hidden)
                
                # ========== 维度对齐（核心！） ==========
                # 1. 扩展logits到seq_len维度，以便对每个动作计算log_prob
                #    因为Categorical需要 [seq_len, action_dim] 的logits
                logits_seq = logits_last.expand(seq_len, -1)  # [seq_len, action_dim]
