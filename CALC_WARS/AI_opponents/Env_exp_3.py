...
class Game:
    ...
    def _get_state(self):
        ...
        if partial_result is None:  #如果是None可能是AI选择了一个符号，比如3+，那就计算前面的部分
            ...
            op_last = True #不用管是否有效，函数里还会继续处理无效情况
        else:
            op_last = False
        ...
        if len(self.selected_cards) > 0:
            ...
            if partial_result is not None:
                distances = [(partial_result - zone) / 40.0 for zone in target_zones]
        ...
        # 部分特征提取
        self.state_vector[124] = self.nonaction_times / 5.0 #无效动作统计
        self.state_vector[125] = (self.nonaction_times  ** 2) / 25.0
    ...
    def get_valid_actions_mask(self):
        ...
        if len(self.selected_cards) == 0:
            ...
            if len(self.selected_cards) >= 2:
            # 检查模式：除法 → 禁止选择数字0
                if ( last_card.card_type == CardType.OPERATOR and 
    ...
    def step(self, action):
        ...
        reward += - 0.5 * self.nonaction_times
        reward += - 1.0 * self.noneffect_times
        reward += time_penalty
