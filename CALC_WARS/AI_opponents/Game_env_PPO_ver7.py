...
class Game:
    def __init__(self, use_curriculum=False):
        ...
        self._state_buffer = np.zeros(104, dtype=np.float32)
        self._mask_buffer = np.zeros(29, dtype=np.int8)
        self._zones_buffer = np.zeros(7, dtype=np.float32)
        
        # 预计算常量
        self._target_weights = np.array([3.0, 2.0, 2.0, 1.0, 1.0, 1.0, 1.0], dtype=np.float32)
    ...
    def _get_state(self):
        ...
        # 符号特征（批量填充）
        if trace and trace[-1][1] is not None:
            state[97] = trace[-1][1] / 60.0   # 当前值
            state[98] = trace[-1][2] / 2.0     # 当前深度
            state[99] = 1.0 if trace[-1][3] else 0.0  # 有效性
        else:
            state[97:100] = 0.0  # 无trace时置零


        state[100] = self.nonaction_times / 3.0
        state[101] = (self.nonaction_times  ** 2) / 9.0
        total_actions = self.continuous_operations + self.nonaction_times + 1
        state[102] = self.nonaction_times / total_actions
        state[103] = 1.0 if self.done else 0.0  # 游戏结束标志
    ...
    def step(self, action):
        ...
        if action < 20 :  # 选择手牌
            if self.current_player.hand[action] > 0:
                ...
                self.symbolic_executor.execute_step(card, len(self.selected_cards) - 1, self.selected_cards)

                # 统一过程奖励
                reward += self._get_progress_reward(self.symbolic_executor.trace)
                reward += self._get_bracket_reward(card, self.symbolic_executor.trace)
            ...
            if len(self.selected_cards) >= 3:
                ...
                if result is not None and result >= 0 and self.is_valid_expression(self.selected_cards):
                    self.nonaction_times = 0
                    ...
                    # 奖励基于造成的伤害
                    reward += 8.0
                    reward += actual_damage * 2.0   #伤害奖励

                    cards_used = len(self.selected_cards)
                    efficiency = actual_damage / max(cards_used, 1)
                    # 区分效率：高伤害时效率奖励更高

                    if actual_damage >= 50:  # 红区
                        reward += efficiency * 3.0
                    elif actual_damage >= 30:  # 黄区
                        reward += efficiency * 1.5
                    else:  # 蓝区或零伤害
                        reward += efficiency * 0.5  # 低伤害时效率奖励更低

                    reward += (cards_used - 3.0) * 0.5

                    red_dist = abs(self.target.red_zone - result)
                    if red_dist <= 5:
                        reward += (5 - red_dist) * 5.0  # 每近1单位+10
                    else:
                        reward += -min(10, red_dist * 0.2)      # 每远1单位-0.2
        ...
        reward += - 3.0 * self.nonaction_times

        # 检查游戏是否结束
        if self.player1.hp <= 0 or self.player2.hp <= 0:
            self.done = True
            if self.current_player.hp > 0:
                reward = 1600  - 1.2 ** (self.step_count / 14.0) # 获胜奖励 鼓励速胜
            else:
                reward = -800.0  # 失败惩罚
    ...
    def _get_progress_reward(self, trace: List[Tuple]) -> float:
        """过程奖励：当前状态到靶子的可达性"""
        if not trace or trace[-1][1] is None:
            return 0.0
        
        if not self.is_valid_expression(self.selected_cards):
            return 0.0
        
        current_val = float(trace[-1][1])
        depth = trace[-1][2]
        
        # 计算所有可能的下一步值（保守估计）
        future_vals = []
        # 数字
        future_vals.extend([current_val + i for i in range(14) if self.current_player.hand[i] > 0])
        # 乘法
        future_vals.extend([current_val * i for i in range(1,14) if self.current_player.hand[i] > 0])
        # 减法
        future_vals.extend([current_val - i for i in range(1,14) if self.current_player.hand[i] > 0])
        
        # 到所有靶区的最小距离
        all_zones = [self.target.red_zone] + self.target.yellow_zone + self.target.blue_zone
        min_future_dist = min(min(abs(v - z) for z in all_zones)for v in future_vals) if future_vals else 1000
        
        return max(0, 10 - min_future_dist) * 1.5 * (0.9 ** len(trace))

    def _get_bracket_reward(self, card: Card, trace: List[Tuple]) -> float:
        """括号专用奖励：鼓励正确嵌套"""
        reward = 0.0
        if card.card_type == CardType.OPERATOR:
            if card.operator_type == OperatorType.LEFTBRA:
                ...
                self.selected_cards[-1].card_type == CardType.OPERATOR and \
                self.selected_cards[-1].operator_type == OperatorType.MULTIPLY:
                    reward += 0.5
            elif card.operator_type == OperatorType.RIGHTBRA:
                # 右括号：奖励匹配成功
                if trace[-1][2] < trace[-2][2]:  # depth下降
                    reward += 1.0
                    # 额外奖励：括号内计算结果提升
                    inner_val = trace[-1][1] if trace[-1][1] else 0
                    outer_val = trace[-2][1] if trace[-2][1] else 0
                    if inner_val > outer_val:
                        reward += (inner_val - outer_val) * 0.2
        return reward
    ...
    @staticmethod
    def calculate_expression(selected_cards: List[Card]) -> Optional[int]:
        """计算当前选中表达式的结果"""
        if not Game.is_valid_expression(selected_cards):
            return None
        
        executor = SymbolicExecutor()
        for i, card in enumerate(selected_cards):
            executor.execute_step(card, i, selected_cards)
        
        return executor.get_final_result()
    ...
    @staticmethod
    def is_valid_expression(cards: List[Card]) -> bool:
        """O(n)状态机验证，无递归"""
        if len(cards) < 3:
            return False
...
class SymbolicExecutor:
    ...
    def execute_step(self, card: Card, step_idx: int, selected_cards: List[Card]) -> Tuple[float, int, bool]:
        ...
        if card.card_type == CardType.NUMBER:
            ...
            if card.operator_type == OperatorType.LEFTBRA:
                ...
                if self.bracket_depth < 0:
                    self._last_state = (self.INVALID_VALUE, 0, False)
                    self.trace.append((step_idx, self.INVALID_VALUE, 0, False))
        ...
        if is_valid and selected_cards:
            is_valid = Game.is_valid_expression(selected_cards)
    ...
    def get_full_trace(self) -> List[Tuple[float, int]]:
        ...
        return [(t[0], t[1] if t[1] is not None else self.INVALID_VALUE, t[2]) for t in self.trace]
    ...
    def get_masked_trace(self, mask_len: int) -> List[Tuple[float, int]]:
        ...
        masked = full[:mask_idx] + [(-1, self.INVALID_VALUE, -1)] + full[mask_idx+1:]
        return masked
