...
class Target:
    def __init__(self):
        ...
        self.blue_zone = random.sample([1,2,3,5,6,7,10,11,12,13,14,15,17,18,19,20,21,22,23],3)+ random.sample([4,9,16],1)
...
class Game:
    ...
    def __init__(self, first_player: str = None, match_mode: str = 'single'):
        ...
        self.is_match = (self.match_mode == 'bo5')
        if self.is_match:
            self.match_scores = {'player1': 0, 'player2': 0}
            self.match_target_wins = 3
            # 初始先手在 match 开始时随机决定（若 menu 提供 first_player 则尊重）
            self.match_starting_player = None
            self.show_scoreboard_overlay = True
        else:
            self.match_scores = {'player1': 0, 'player2': 0}
            self.match_target_wins = 1
            self.match_starting_player = None
            self.show_scoreboard_overlay = False
        ...
        # Match 覆盖启动按钮
        self.match_start_button = Button(SCREEN_WIDTH//2 - 100, SCREEN_HEIGHT//2 + 60, 200, 60, "Start Match", (100,255,100), (150,255,150))

        # 初始化游戏
        if not self.is_match:
            self.initial_deal(first_player)
        else:
            # BO5 模式不立即开始第一局，展示计分板覆盖层，先手由覆盖层开始时决定（若 menu 已指定 first_player，则使用之）
            if first_player == 'player1':
                self.match_starting_player = self.player1
            elif first_player == 'player2':
                self.match_starting_player = self.player2
            else:
                self.match_starting_player = None
            self.show_scoreboard_overlay = True
    ...
    def _play_win_sound_once(self):
        """确保胜利音效只播放一次（使用保留的专用声道播放，避免被其他音效占用导致回退）"""
        if 'win' not in self.sound_effects:
            print("[Audio] win sound not loaded")
            return
        s = self.sound_effects['win']
        try:
            # 确保 mixer 有足够通道，并保留第 0 号通道专用于胜利音效
            try:
                needed = 16
                current = pygame.mixer.get_num_channels()
                if current < needed:
                    pygame.mixer.set_num_channels(needed)
                pygame.mixer.set_reserved(1)  # 保留第一个通道
            except Exception as e:
                print(f"[Audio] Channel reservation/setup failed (non致命): {e}")

            # 使用保留的第 0 号通道来播放胜利音效
            try:
                self._win_channel = getattr(self, '_win_channel', None) or pygame.mixer.Channel(0)
                try:
                    self._win_channel.stop()
                except Exception:
                    pass
                self._win_channel.set_volume(self.sfx_volume)
                self._win_channel.play(s)
                print("[Audio] Played win sound on reserved channel")
                return
            except Exception as e:
                print(f"[Audio] Reserved channel play failed: {e}")

            # 如果保留通道不可用，尝试 find_channel() 再回退到直接播放
            ...
            if ch:
                ...
                ch.play(s)
                print("[Audio] Played win sound on dedicated channel (fallback-find)")
            else:
                try:
                    s.stop()
                except Exception:
                ...
                s.play()
                print("[Audio] Played win sound via Sound.play() fallback")
        except Exception as e:
            print(f"[Audio] Failed to play win sound overall: {e}")
    ...
    def load_font(self, size):
        try:
            font_names = ['times new roman', 'Times New Roman', 'Arial']
    ...
    def initial_deal(self, first_player: str = None):
        """初始发牌（参考 AI 规则：5 张数字 + 2 张符号，并对后手玩家补偿一个 RUIN 技能）"""
        for _ in range(5):
    ...
    # ---------- Match / BO5 helpers ----------
    def _draw_match_scoreboard(self):
        if not self.is_match:
            return
        score_text = f"BO5 Score - P1: {self.match_scores['player1']}  P2: {self.match_scores['player2']}"
        score_surf = self.large_font.render(score_text, True, GOLD_CLASSIC)
        self.screen.blit(score_surf, (830, 30))

    def _reset_for_next_round(self):
        """重置状态，为下一回合做准备（不自动开始下一局，等待玩家点击 Start Match）"""
        # 恢复血量和护盾
        self.player1.hp = 120
        self.player2.hp = 120
        ...
        self.player1.hand = []
        self.player2.hand = []
        self.player1.skill_cards = []
        self.player2.skill_cards = []
        # 新目标
        self.target = Target()
        # 重置其他回合变量
        self.selected_cards = []
        ...
        self.player2_round_end = False
        self.continuous_operations = 0
        # 确保 match_starting_player 有值（若无则随机）
        if self.match_starting_player is None:
            self.match_starting_player = random.choice([self.player1, self.player2])
        # 不自动发牌：显示 scoreboard 覆盖层，等待玩家点击 Start Match
        self.show_scoreboard_overlay = True

    def _on_round_victory(self, winner_player):
        """处理一局结束时的比赛计分逻辑；返回 True 表示整场比赛结束"""
        # 如果这是 BO5 的终局胜利，直接显示终局消息（避免与分局胜利消息重叠）
        if self.is_match:
            winner_key = 'player1' if winner_player == self.player1 else 'player2'
            # 如果再得一分就达到胜利目标，视为终局胜利
            if self.match_scores[winner_key] + 1 >= self.match_target_wins:
                # 更新比分以反映最终结果
                self.match_scores[winner_key] += 1
                msg = f"Round Over! Score {self.match_scores['player1']} - {self.match_scores['player2']}"
                self.add_action_message(msg)
                self.message = msg
                # 展示终局胜利信息并播放音效（只展示一次）
                if winner_key == 'player1':
                    final = "Player1 Wins the Match!"
                else:
                    final = "Player2 Wins the Match!"
                win_font = pygame.font.SysFont('Arial', 240, bold=True)  # 使用大字体突出终局
                win = win_font.render(final, True, ORANGE_PEACH)
                self.screen.blit(win, (SCREEN_WIDTH//2 - win.get_width()//2, SCREEN_HEIGHT//2 - 80))
                pygame.display.flip()
                self._play_win_sound_once()
                pygame.time.wait(3500)
                return True

        # 非终局或普通单局：先立即展示本局胜者的大字（与 single 一样大的字体）并播放一次胜利音效
        if winner_player == self.player1:
            win_text = "Player1 WINS!"
        else:
            win_text = "Player2 WINS!"
        win_font = pygame.font.SysFont('Arial', 240, bold=True)  # 与 single 相同的大字体
        win_surf = win_font.render(win_text, True, ORANGE_PEACH)
        self.screen.blit(win_surf, (SCREEN_WIDTH//2 - win_surf.get_width()//2, SCREEN_HEIGHT//2 - 80))
        pygame.display.flip()
        # 播放一次胜利音效并等待 2.2 秒（让玩家感受胜利时刻）
        self._play_win_sound_once()
        pygame.time.wait(2200)

        # 更新比分并在侧栏添加消息
        if winner_player == self.player1:
            self.match_scores['player1'] += 1
        else:
            self.match_scores['player2'] += 1
        ...
        # 检查是否整场比赛结束
        if self.match_scores['player1'] >= self.match_target_wins or self.match_scores['player2'] >= self.match_target_wins:
            # 比赛结束，展示最终胜者
            if self.match_scores['player1'] > self.match_scores['player2']:
                final = "Player1 Wins the Match!"
            else:
                final = "Player2 Wins the Match!"
            win_font = pygame.font.SysFont('Arial', 120, bold=True)
            win = win_font.render(final, True, ORANGE_PEACH)
            self.screen.blit(win, (SCREEN_WIDTH//2 - win.get_width()//2, SCREEN_HEIGHT//2 - 80))
            pygame.display.flip()
            self._play_win_sound_once()
        ...
        # 否则切换下一局先手，准备下一局但不自动开始（显示 Start Match 覆盖层）
        if self.match_starting_player == self.player1:
            self.match_starting_player = self.player2
        else:
            self.match_starting_player = self.player1
    ...
    def end_round(self):
        """结束当前轮次 双方都结束本轮后调用"""
        # 下一轮先手权给当前轮次先结束本轮的玩家
        # Align with AI: 增加 turn_number 并在被补偿的一方发放 3 张补偿牌
        self.turn_number = getattr(self, 'turn_number', 1) + 1
    ...
    def handle_click(self, pos):
        ...
        # BO5 启动覆盖界面点击处理
        if self.show_scoreboard_overlay:
            # 如果点击了开始按钮，开始第一局
            if self.match_start_button.rect.collidepoint(x, y):
                self.show_scoreboard_overlay = False
                # 决定首发（如果尚未决定则随机），并发牌
                if self.match_starting_player is None:
                    self.match_starting_player = random.choice([self.player1, self.player2])
                first = 'player1' if self.match_starting_player == self.player1 else 'player2'
                self.initial_deal(first)
                return
            # 覆盖层显示时不处理其他点击
            return
    ...
    def draw(self):
        ...
        if self.show_scoreboard_overlay:
            ...
            board_rect = pygame.Rect(SCREEN_WIDTH//2 - 300, SCREEN_HEIGHT//2 - 160, 600, 320)
            pygame.draw.rect(self.screen, (245, 245, 240), board_rect)
            pygame.draw.rect(self.screen, (0,0,0), board_rect, 3)

            title = self.large_font.render('BO5 Match - First to 3 wins', True, (10,10,10))
            self.screen.blit(title, (board_rect.centerx - title.get_width()//2, board_rect.y + 20))

            score_text = f"Score: P1 {self.match_scores['player1']}  -  P2 {self.match_scores['player2']}"
            score_surf = self.large_font.render(score_text, True, GOLD_CLASSIC)
            self.screen.blit(score_surf, (board_rect.centerx - score_surf.get_width()//2, board_rect.y + 100))

            # 鼠标悬停状态更新
            self.match_start_button.check_hover(mouse_pos)
            self.match_start_button.draw(self.screen)
    ...
    def run(self):
        ...
        while running:
            ...
            if self.player1.hp <= 0 or self.player2.hp <= 0:
                ...
                if self.is_match:
                    match_over = self._on_round_victory(winner)
                    if match_over:
                        running = False
                    else:
                        # 继续下一局，循环继续
